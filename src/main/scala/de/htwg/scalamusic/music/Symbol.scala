...
trait Symbol extends MusicElem {
  ...
  override def play(
      ...
      volume: Int
  ) = {} // play nothing for Sympols
  def *(repetitions: Int) = this
  def *(pattern: Pattern) = this

}

case class TimeSignature(nominator: Int = 4, denominator: Int = 4)
    extends Symbol {
...
case class Pause(override val ticks: Int = 4) extends Symbol {
  override def play(instrument: Instrument = Piano, volume: Int = 0) =
    instrument.midiPlayer.play(key = 1, duration = duration, volume = 0)
