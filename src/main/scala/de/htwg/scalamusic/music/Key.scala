...
case class Key(
    ...
    def play(instrument: Instrument = Piano, volume: Int = volume): Unit =
        for (i <- 1 to repeat; part <- pattern) {
            instrument.midiPlayer.play(
              key = midiNumber,
              duration = duration,
              volume = volume * part
            )
    ...
    def toTickList: List[Option[Music]] =
        (1 to repeat).toList.flatMap(x =>
            pattern.flatMap(part => Some(this.copy(volume = volume * part)) :: ((1 until ticks).toList.map(x => None)))
        )
    ...
    def chord(chordQuality: ChordQuality): Chord =
        chordQuality match {
            case ChordQuality.Major =>
                Chord(
                  Set(this, majorTerz, majorQuint),
                  name = toString.toUpperCase() + "maj"
                )
            case ChordQuality.Minor =>
                Chord(
                  Set(this, minorTerz, majorQuint),
                  name = toString.toUpperCase() + "min"
                )
            case ChordQuality.Diminshed =>
                Chord(
                  Set(this, minorTerz, minorQuint),
                  name = toString.toUpperCase() + "dim"
                )
            case ChordQuality.Augmented =>
                Chord(
                  Set(this, majorTerz, augmentedQuint),
                  name = toString.toUpperCase() + "aug"
                )
            case ChordQuality.Seventh =>
                Chord(
                  Set(this, majorTerz, majorQuint, minorSetp),
                  name = toString.toUpperCase() + "7"
                )
            case ChordQuality.MajorSeventh =>
                Chord(
                  Set(this, majorTerz, majorQuint, majorSetp),
                  name = toString.toUpperCase() + "maj7"
                )
            case ChordQuality.MinorSeventh =>
                Chord(
                  Set(this, minorTerz, majorQuint, minorSetp),
                  name = toString.toUpperCase() + "min7"
                )
        }
    ...
    def scale(scaleType: ScaleType = ScaleType.Major) = Scale(this, scaleType)
    ...
    override def toString =
        if (midiNumber == 128) "|"
            ...
            keynumberToString(keynumber) + octaveToString(octave) + ticksToString(ticks)

    override def equals(that: Any): Boolean =
        that match {
            case that: Key =>
                (this.midiNumber == that.midiNumber) && (this.ticks == that.ticks)
            case _ => false
        }
