...
case class Chord(
  ...
  def play(instrument: Instrument = Piano, volume: Int): Unit = for (
  ...
  override def toString =
    if (name == "") "[" + set.foreach(_.toString) + "]" else name
  ...
  def toTickList = (1 to repeat).toList.flatMap(x =>
    pattern.flatMap(part =>
      Some(this.copy(volume = volume * part)) :: ((1 until ticks).toList.map(
        x => None
      ))
    )
  )
...
object ChordQuality extends Enumeration {
  ...
  val Major, Minor, Diminshed, Augmented, Seventh, MajorSeventh, MinorSeventh =
    Value
}
