...
case class Chord(
    ...
    def toTickList = (1 to repeat).toList.flatMap(x =>
        pattern.flatMap(part => Some(this.copy(volume = volume * part)) :: ((1 until ticks).toList.map(x => None)))
    )

object Chord:
    def apply(keys: Key*): Chord = Chord(keys.toSet)

    def apply(rootkey: Key, chordQuality: ChordQuality): Chord =
        rootkey.chord(chordQuality)

enum ChordQuality:
    case Major, Minor, Diminshed, Augmented, Seventh, MajorSeventh, MinorSeventh
